...
-  Host Aggregates:
   ...
   -  Host Aggregates represent a logical set of
      ...
      desired property. Host Aggregates are managed by OpenStack admins.
...
| Create two Host Aggregate called ``"az1-ag"/"az2-ag"``, this command
  also, will create two Availability Zones called ``"az1"/"az2"``.
| By default, when a Host Aggregate is created with an Availability
  Zone, a metadata key called ``"availability_zone=NAME_OF_AZ``" will be
  created.
...
| By default, Nova Scheduler don't allow filtering by extra Specs
  inserted in flavors or images.
